    ...
    <div class="overflow-auto">
      ...
      >
        ...
        <template #cell-group="{ row }">
          <span v-if="row.group" class="inline-flex items-center rounded px-2 py-0.5 text-xs font-medium bg-info-soft text-info-deep dark:bg-info/20 dark:text-info">
            {{ row.group.name }}
          </span>
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-else class="flex items-center gap-1.5">
            <div class="space-y-1 text-sm">
              <div class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1 cursor-help" :title="t('usage.tokenIconHint.input')">
                  <Icon name="arrowDown" size="sm" class="h-3.5 w-3.5 text-success" />
                ...
                <div class="inline-flex items-center gap-1 cursor-help" :title="t('usage.tokenIconHint.output')">
                  <Icon name="arrowUp" size="sm" class="h-3.5 w-3.5 text-brand-500" />
              ...
              <div v-if="row.cache_read_tokens > 0 || row.cache_creation_tokens > 0" class="flex items-center gap-2">
                <div v-if="row.cache_read_tokens > 0" class="inline-flex items-center gap-1 cursor-help" :title="t('usage.tokenIconHint.cacheRead')">
                  <svg class="h-3.5 w-3.5 text-info" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>
                  <span class="font-medium text-info dark:text-info">{{ formatCacheTokens(row.cache_read_tokens) }}</span>
                </div>
                <div v-if="row.cache_creation_tokens > 0" class="inline-flex items-center gap-1 cursor-help" :title="t('usage.tokenIconHint.cacheWrite')">
                  ...
                  <span class="font-medium text-warning dark:text-brand-300">{{ formatCacheTokens(row.cache_creation_tokens) }}</span>
                  <span v-if="row.cache_creation_1h_tokens > 0" :title="t('usage.tokenIconHint.cacheTtl1h')" class="inline-flex items-center rounded px-1 py-px text-2xs font-medium leading-tight bg-warning-soft text-warning ring-1 ring-inset ring-warning/30 dark:bg-warning/20 dark:text-brand-300 dark:ring-warning/30 cursor-help">1h</span>
                  <span v-if="row.cache_ttl_overridden" :title="t('usage.cacheTtlOverriddenHint')" class="inline-flex items-center rounded px-1 py-px text-2xs font-medium leading-tight bg-danger-soft text-danger ring-1 ring-inset ring-danger/30 dark:bg-danger/20 dark:text-danger dark:ring-danger/30 cursor-help">R</span>
                </div>
              </div>
            </div>
            ...
            >
              <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-gray-100 transition-colors group-hover:bg-info-soft dark:bg-dark-700 dark:group-hover:bg-info/40">
                <Icon name="infoCircle" size="xs" class="text-gray-400 group-hover:text-info dark:text-gray-500 dark:group-hover:text-info" />
              </div>
            </div>
          </div>
        </template>
        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-success dark:text-tea-300">${{ row.actual_cost?.toFixed(6) || '0.000000' }}</span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
              ...
              >
                <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-gray-100 transition-colors group-hover:bg-info-soft dark:bg-gray-700 dark:group-hover:bg-info/40">
                  <Icon name="infoCircle" size="xs" class="text-gray-400 group-hover:text-info dark:text-gray-500 dark:group-hover:text-info" />
                </div>
              </div>
            </div>
            <div v-if="row.account_rate_multiplier != null" class="mt-0.5 text-2xs text-brand-500 dark:text-brand-400">
      ...
      <div class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-gray-600 dark:bg-gray-800">
        <div class="space-y-1.5">
          <div>
              ...
              <template v-if="tokenTooltipData.cache_creation_5m_tokens > 0 || tokenTooltipData.cache_creation_1h_tokens > 0">
                ...
                <div v-if="tokenTooltipData.cache_creation_1h_tokens > 0" class="flex items-center justify-between gap-4">
                  <span class="text-gray-400 flex items-center gap-1.5">
                    ...
                    <span class="inline-flex items-center rounded px-1 py-px text-2xs font-medium leading-tight bg-warning-deep/20 text-warning-deep ring-1 ring-inset ring-warning-deep/30">1h</span>
                  </span>
            ...
            <div v-if="tokenTooltipData && tokenTooltipData.cache_ttl_overridden" class="flex items-center justify-between gap-4">
              <span class="text-gray-400 flex items-center gap-1.5">
                ...
                <span class="inline-flex items-center rounded px-1 py-px text-2xs font-medium leading-tight bg-danger/20 text-danger ring-1 ring-inset ring-danger/30">R-{{ tokenTooltipData.cache_creation_1h_tokens > 0 ? '5m' : '1H' }}</span>
              </span>
          ...
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            ...
            <span class="font-semibold text-info">{{ ((tokenTooltipData?.input_tokens || 0) + (tokenTooltipData?.output_tokens || 0) + (tokenTooltipData?.cache_creation_tokens || 0) + (tokenTooltipData?.cache_read_tokens || 0)).toLocaleString() }}</span>
          </div>
        </div>
      ...
      <div class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-gray-600 dark:bg-gray-800">
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <template v-if="!tooltipData?.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN">
              <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-info">{{ formatTokenPricePerMillion(tooltipData.input_cost, tooltipData.input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            ...
            <template v-else-if="tooltipData?.billing_mode === BILLING_MODE_IMAGE">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-info">${{ imageUnitPrice(tooltipData).toFixed(6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
            ...
            <div v-else class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-info">${{ tooltipData?.total_cost?.toFixed(6) || '0.000000' }}</span>
            </div>
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-info">{{ getUsageServiceTierLabel(tooltipData?.service_tier, t) }}</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-gray-400">{{ t('usage.rate') }}</span>
            <span class="font-semibold text-info">{{ formatMultiplier(tooltipData?.rate_multiplier || 1) }}x</span>
          </div>
          <div class="flex items-center justify-between gap-6">
          ...
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            ...
            <span class="font-semibold text-info">{{ formatMultiplier(tooltipData?.account_rate_multiplier ?? 1) }}x</span>
          </div>
          <div class="flex items-center justify-between gap-6">
