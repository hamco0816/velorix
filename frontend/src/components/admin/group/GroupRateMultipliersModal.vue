    ...
    <div v-if="group" class="space-y-4">
      ...
      <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-gray-100 pt-3 dark:border-dark-600">
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-lg border border-danger/30 bg-danger-soft px-3 py-1.5 text-sm font-medium text-danger transition-colors hover:bg-danger/10 dark:border-danger/40 dark:bg-danger/20 dark:text-danger dark:hover:bg-danger-deep/40"
              @click="clearAllLocal"
            >
              ...
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-600 dark:bg-dark-700">
                    ...
                    <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">ID</th>
                    ...
                    <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                    <th v-if="showFinalRate" class="px-3 py-2 text-right text-xs font-medium text-primary-600 dark:text-primary-400">{{ t('admin.groups.finalRate') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-600">
                  ...
                  >
                    ...
                    <td class="whitespace-nowrap px-3 py-2 text-right tabular-nums text-gray-400 dark:text-gray-500">{{ entry.user_id }}</td>
                    ...
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 text-right tabular-nums font-medium text-primary-600 dark:text-primary-400">
                      {{ computeFinalRate(entry.rate_multiplier) }}
                    </td>
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-gray-500 transition-colors hover:bg-danger-soft hover:text-danger dark:text-dark-400 dark:hover:bg-danger-deep/20 dark:hover:text-danger"
      ...
      <div class="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <template v-if="isDirty">
          <span class="text-xs text-warning dark:text-warning">{{ t('admin.groups.unsavedChanges') }}</span>
          <button
            type="button"
