<template>
  <div ref="rootRef" class="relative">
    ...
    <transition name="support-pop">
      <section
        v-if="open"
        class="fixed inset-x-2 bottom-3 top-20 z-50 flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-2xl dark:border-dark-700 dark:bg-dark-900 sm:absolute sm:inset-x-auto sm:bottom-auto sm:right-0 sm:top-full sm:mt-2 sm:h-[560px] sm:w-[380px]"
      >
        ...
        <div ref="messageListRef" class="flex-1 space-y-3 overflow-y-auto bg-gray-50 px-4 py-4 dark:bg-dark-950">
...
function handleRealtime(event: SupportRealtimeEvent) {
  ...
  if (event.type === 'support.message' && event.message) {
    ...
    if (event.message.sender_type === 'admin' && !open.value) {
