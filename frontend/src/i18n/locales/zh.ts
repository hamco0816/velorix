  ...
  admin: {
    ...
    pricingAdvisor: {
      ...
      calculator: {
        ...
        usersPerAccountTip: '你打算让多少个订阅用户共用一个账号。系统把上游 cap × 安全系数后均分给这 N 个用户，得每个用户的 5h / 日 / 周 / 月限额；月费 = 账号成本 × (1+加价率) ÷ (N × 上座率)。',
        markup: '加价率',
        ...
        profit: '预期月利润',
        profitTip: '按上座率算的月利润。满座（100%）月利润：¥{full}。低于 0 表示按当前定价 + 上座率不足以覆盖账号成本。',
        perMonth: '月',
        limitsTitle: '每人订阅限额（USD）',
        limitsTip: '5h 限额 = cap_5h × safety / N；周限额 = cap_7d × safety / N；日 = 周/7；月 = 周×4。cap 优先用 utilization 反推得到，没采样到时回退用 7d_P95 估计。',
        limitsSourceReversed: '反推自：cap_5h={cap5h}$ · cap_7d={cap7d}$（{samples} 个账号 utilization 反推中位数），× safety {safety} ÷ N={n}',
        limitsSourceFallback: '该档位无 utilization 采样，回退用 7d_P95 当 cap：cap_5h≈{cap5h}$ · cap_7d≈{cap7d}$，× safety {safety} ÷ N={n}',
        limitsCapsIndependentNote: '注：4 条限额是独立约束，用户实际撞到的是其中最严的那个。',
        applyToPlan: '用此建议创建套餐',
        warnNoCapSample: '该档位没有 utilization 采样可反推 cap，当前结果用 7d_P95 兜底估计。等账号多跑一段时间，被动采样收集到 utilization 后会更准。',
