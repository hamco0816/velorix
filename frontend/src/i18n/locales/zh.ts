export default {
  ...
  admin: {
    ...
    pricingAdvisor: {
      ...
      helpBody: '计算器优先用「上游 cap 反推」：cap = 当前窗口已耗费 USD ÷ 上游 utilization%。例如 7d 窗口当前 50%、消耗 $10，则 cap = $20。再按 N 个用户均分，每用户周限额 = cap × safety / N，日限额 = 周限额 / 7，月限额 = 周限额 × 4。表格里的 5h / 7d 是过去 N 天的滑窗峰值，作为参考；当某档位无 utilization 采样时，cap 回退用 7d_P95 估计。',
      ...
      tip: {
        samples: '该档位下有数据的账号个数。≥ 3 个样本时统计才有意义；样本少时 P95 会被 ⚠ 标注。',
        ...
        dailyAvg: '过去 N 天每账号每日成本均值。粗略代表"稳态消耗"。',
      ...
      calculator: {
        ...
        usersPerAccountTip: '你打算让多少个订阅用户共用一个账号。系统把上游 cap × 安全系数后均分给这 N 个用户，得每个用户的 5h / 日 / 周 / 月限额；月费/人 = 账号成本 × (1+加价率) ÷ (N × 上座率)。',
        markup: '加价率',
        markupTip: '加价率 = 利润 / 成本。例：账号成本 ¥1400、加价率 30% → 总收入目标 ¥1820。注意这不是利润率（利润率 = 利润/收入 = 23%）。',
        occupancy: '上座率',
        occupancyTip: '预计每个账号 N 个座位实际能卖出去的比例。低于 100% 时定价会按"少卖几个仍要回本"算高一些。例：N=10、上座率 80% → 按 8 个用户分摊成本。',
        safety: '安全系数',
        safetyTip: '< 1 = 按 cap 的百分比定额，越小越保守、撞顶风险越低，但用户可能感觉"不够用"；建议 0.7-0.85。',
        rateMultiplier: '套餐倍率',
        rateMultiplierTip: '该套餐分组的 rate_multiplier。用户实际扣费 = 上游成本 × 倍率，所以倍率 > 1 时用户侧限额需要相应放大。默认 1（不改的话不影响计算）。',
        capOverrideTitle: '高级：手动覆盖档位 cap',
        capOverrideHint: '系统会按 utilization 反推 cap，没采到时用 7d_P95 兜底。如果你知道该档位的上游真实 cap（例如官方文档写明 Pro 20x 周限 $4000），可以在这里直接填入覆盖。留空走自动。',
        ...
        profit: '预期月利润',
        profitTip: '按上座率算的月利润 = 月费/人 × N × 上座率 − 账号成本。满座（100%）月利润：¥{full}。低于 0 表示当前定价 + 上座率不足以覆盖账号成本。',
        perMonth: '月',
        limitsTitle: '每人订阅限额（USD）',
        limitsTip: '5h 限额 = cap_5h × safety × 套餐倍率 / N；周 = cap_7d × safety × 套餐倍率 / N；日 = 周/7；月 = 周×4。cap 优先按"手动覆盖 > utilization 反推 > 7d_P95 兜底"取值。',
        limitsSourceLine: 'cap_5h={cap5h}$（{cap5hSrc}）· cap_7d={cap7d}$（{cap7dSrc}），× safety {safety} × 倍率 {mul} ÷ N={n}',
        capSource_override: '手动',
        capSource_reversed: 'utilization 反推',
        capSource_fallback: '7d_P95 兜底',
        limitsCapsIndependentNote: '注：4 条限额是独立约束，用户实际撞到的是其中最严的那条。5h 仅参考，目前套餐表不存档 5h，需要时去上游订阅那边卡。',
        applyToPlan: '用此建议创建套餐',
        warnNoCapSample: '该档位没有 utilization 采样可反推 cap，当前结果用 7d_P95 兜底估计。如果账号没被跑满，兜底值会显著低于真实 cap — 建议在"高级"里手动填入真实 cap。',
        warnSamplesLow: '样本不足（< 3 个账号），建议结果仅供参考，等多积累几个账号再决策。',
        warnNTooHigh: '单账号承载用户数过多，单人限额会非常小，用户体验会受影响，建议适当下调。',
        warnUnprofitable: '按当前定价 + 上座率，预计无法覆盖账号成本。要么提高加价率，要么提高上座率预期，要么调高 N。',
      },
