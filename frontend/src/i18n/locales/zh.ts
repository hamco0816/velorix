export default {
  ...
  auth: {
    ...
    turnstileLoading: '正在验证身份…',
    turnstileVerified: '已通过验证',
    turnstileLoadSlow: '人机验证加载较慢，可能是网络原因。',
    turnstileRetry: '点击重试',
  ...
  pricing: {
    ...
    imagePer2K: '2K 每张',
    imagePer4K: '4K 每张',
    imageRateNote: '当前分组「图片倍率独立」已启用，以上价格已按 {rate} 折算',
    noPricing: '未配置定价',
    tiered: '阶梯',
    tieredHint: '此模型按 token 区间分档定价',
    availableIn: '可用于（点击切换到该分组价格）',
    switchToGroup: '切换到「{name}」分组价格',
    billingToken: '按量计费',
    ...
    cnyHint: '按 $1 ≈ ¥{rate} 换算',
    cnyHint2: '您实付的人民币金额',
    usdHint: '按外汇汇率换算的真实美元（$1 ≈ ¥{rate}）',
    promoActive: '限时优惠',
    promoCountdownDays: '{d}天 {h}:{m}:{s}',
    standardModel: '标准',
  ...
  imageGen: {
    title: '生图工坊',
    subtitle: '用 prompt 直接生成图片，无需写代码。当前为体验版，生成结果不会保留，请及时下载。',
    promptPlaceholder: '描述你想生成的图片，例：一只穿着宇航服的橙色小猫坐在月球上看地球，写实风格',
    countLabel: '张',
    aspectAuto: '比例 自动',
    qualityLow: '清晰度 低',
    qualityMedium: '清晰度 标准',
    qualityHigh: '清晰度 高清',
    noKeyError: '你还没有创建 API 密钥。',
    noKeyForGroup: '所选分组「{group}」还没有可用的 API 密钥。',
    createKeyHere: '一键创建密钥并继续',
    creatingKey: '创建中…',
    referenceUpload: '上传参考图（以图生图）',
    referenceShort: '参考图',
    referencePrivacyHint: '仅用于本次生成，不会保留在我们的服务器',
    referenceTooLarge: '参考图过大（>4MB），请压缩后再上传',
    referenceTooMany: '最多上传 {n} 张参考图',
    referenceRemove: '移除',
    submitTooltip: '生成（⌘/Ctrl + Enter）',
    resultsTitle: '生成结果',
    previewHint: '正在逐步绘制，完成后会替换为最终高清图，请勿现在保存',
    download: '下载',
    open: '查看原图',
    emptyHint: '填好提示词后点右侧 ↑ 按钮生成',
    notSavedHint: '当前为体验版，生成结果不会保留，刷新或离开页面即丢失，需要保留请下载。',
    errorHeader: '生成失败',
    errorRetry: '重新生成',
    // 状态文字：按事件 + 时间兜底切换
    statusPrep: '准备中',
    statusStart: '生成中',
    statusRefine: '细化中',
    statusRefineWithCount: '细化中 {idx}/{total}',
    statusFinal: '渲染收尾',
    statusDone: '完成',
    elapsed: '已用 {sec} 秒',
    // 左侧分组栏
  ...
  admin: {
    ...
    groups: {
      ...
      activeSubscriptionsHint: '该分组下未过期的活跃订阅数',
      form: {
        name: '名称',
        description: '描述',
        platform: '平台',
        ...
        invoiceEligible: '该分组消费可开票',
        invoiceEligibleHint: '勾选后，用户在本分组的真实消费（余额按量 + 套餐购买）才计入可开票额度。默认不勾选——自对接的低利润分组开票会亏本，应保持关闭。',
        promoTitle: '限时倍率（活动期间自动生效）',
        promoRate: '限时费率倍数',
        promoRatePlaceholder: '如 0.5（半价）',
        promoRateHint: '活动期间用此倍率替代上面的费率',
        promoLabel: '活动名称',
    ...
    pricingAdvisor: {
      title: '订阅定价助手',
      subtitle: '基于历史用量数据，按账号档位估算 7 天滚动窗口峰值成本，辅助你给订阅套餐合理定额、定价。',
      daySuffix: '天',
      helpTitle: '数据怎么来的，怎么用',
      helpBody: '计算器优先用「上游 cap 反推」：cap = 当前窗口已耗费 USD ÷ 上游 utilization%。例如 7d 窗口当前 50%、消耗 $10，则 cap = $20。表格右侧的「日 / 周 / 月限额」是单账号档位上限：周限额 = cap_7d，日限额 = cap_7d ÷ 7，月限额 = cap_7d × 30 / 7（按 30 天）。计算器再按 N 个用户均分这些上限。表格里的 5h / 7d 分位是过去 N 天的滑窗峰值，仅作参考；当某档位无 utilization 采样时，cap 回退用 7d_P95 估计。',
      helpUsdNote: '这里的 USD 是「token 等价美元值」（按 token 单价折算），不是你给账号付的真钱。给套餐配额时直接套用即可，因为系统的套餐限额也是同一个单位。',
      tableTitle: '各档位用量统计（USD）',
      col: {
        platform: '平台',
        tier: '档位',
        samples: '样本数',
      ...
      tip: {
        samples: '该档位下有数据的账号个数。≥ 3 个样本时统计才有意义；样本少时 P95 会被标注为不可靠。',
        '5h': '滚动 5 小时窗口的峰值成本：扫描过去 N 天里每个账号"任意连续 5 小时"的最高累计成本，再按档位聚合分位。反映"用户在 5h 内能跑到多高"。',
        '7d': '滚动 7 天窗口的峰值成本：同上，窗口换成 7×24 小时。反映"用户在一周内能跑到多高"。',
        '5hP95': '过去 N 天，每账号 5 小时滑窗峰值的 95 分位（USD）。鼠标悬停可见 P50 / Max。',
        '7dP95': '过去 N 天，每账号 7 天滑窗峰值的 95 分位（USD）。鼠标悬停可见 P50 / Max。',
        limits: '单账号档位上限（N=1）：周 = cap_7d；日 = cap_7d ÷ 7；月 = cap_7d × 30 ÷ 7。cap 优先用 utilization 反推，缺失时回退 7d_P95（偏低）。给 N 个用户共用时，在下方计算器里除以 N。',
        p50: '50% 分位（中位数）：把组内账号峰值排序后取中间值。作为典型水平。',
        p95: '95% 分位：组内 95% 账号低于此值。作为"高频上限"参考，更激进的定额。',
        max: '该档位下最重账号的峰值。一般不作为定额依据，仅供观察异常值。',
      },
      samplesLow: '样本少于 3 个，P95 仅供参考',
      empty: {
        title: '暂无用量数据',
        description: '账号开始有流量后，统计会自动填充。建议先在账号管理页给账号打上「订阅档位」标签。',
      },
      samples: '样本',
      calculator: {
        title: '限额计算器',
        tier: '档位',
        usersPerAccount: '单账号承载用户数',
        usersPerAccountTip: '你打算让多少个订阅用户共用一个账号。系统把上游 cap 均分给这 N 个用户，得每个用户的日 / 周 / 月 / 5h 限额。N=1 时就是单账号档位上限。',
        selectFirst: '请先选择档位，再填写参数。',
        tierCapTitle: '档位 cap（上游容量）',
        capSource_override: '手动覆盖',
        capSource_reversed: 'utilization 反推',
        capSource_fallback: '7d_P95 兜底（不准）',
        limitsTitle: '每人限额（USD）',
        limitsTip: '5h 限额 = cap_5h × 套餐倍率 / N；周 = cap_7d × 套餐倍率 / N；日 = 周 ÷ 7；月 = 周 × 30 ÷ 7（按 30 天）。',
        limitsCapsIndependentNote: '4 条限额是独立约束，用户实际撞到的是其中最严的那条。5h 仅参考，套餐表不存 5h，需要时去上游订阅那边卡。',
        suggested5h: '5h 限额',
        suggested5hRefMark: '（参考）',
        suggestedDaily: '日限额',
        suggestedWeekly: '周限额',
        suggestedMonthly: '月限额',
        advancedTitle: '高级参数（默认无需改）',
        advancedHint: '反推 cap 偏低时用手动覆盖；套餐有 rate_multiplier ≠ 1 时用倍率换算。',
        capOverride5h: '5h cap 覆盖（USD）',
        capOverride7d: '7d cap 覆盖（USD）',
        capOverrideTip: '系统按 utilization 反推 cap，没采到时用 7d_P95 兜底（往往偏低）。知道真实 cap 就直接填，留空走自动。',
        rateMultiplier: '套餐倍率',
        rateMultiplierTip: '套餐分组的 rate_multiplier。用户实际扣费 = 上游成本 × 倍率，倍率 > 1 时用户侧限额需放大。默认 1。',
        pricingTitle: '定价（可选）',
        pricingHint: '如果只是想知道限额，这部分可以不填。这里算建议月费和满座利润，公式：月费/人 = 成本 × (1+加价率) / N；月利润 = 月费 × N − 成本。',
        cost: '账号成本（¥/月）',
        markup: '加价率',
        markupTip: '加价率 = 利润 / 成本。例：成本 ¥1400、加价率 30% → 总收入 ¥1820，再除以 N。',
        suggestedPrice: '建议月费/人',
        profit: '满座月利润',
        pricingFormula: '公式：¥{cost} × (1+{markup}%) ÷ N({n}) = 建议月费/人；利润 = 月费 × N − ¥{cost}',
        perMonth: '月',
        applyToPlan: '用此建议创建套餐',
        warnNoCapSample: '该档位没有 utilization 采样可反推 cap，当前用 7d_P95 兜底估计。账号没被跑满时兜底值会显著低于真实 cap，建议在"高级参数"里手动填入。',
        warnSamplesLow: '样本不足（< 3 个账号），建议结果仅供参考，等多积累几个账号再决策。',
        warnNTooHigh: '单账号承载用户数过多，单人限额会非常小，用户体验会受影响，建议适当下调。',
      },
      trendTitle: '过去 N 天日均成本曲线（按档位）',
      trendEmpty: '没有可绘制的趋势数据。',
    },

    pricing: {
      title: '模型定价',
      subtitle: '查看从 GitHub 拉取的全量模型默认定价，并按选定的分组/账号倍率叠加换算"实际成交价"。',
      source: '数据来源',
      sourceUnknown: '未配置远程源（使用本地回退）',
      lastUpdated: '上次更新',
      modelCount: '模型数',
      viewMode: '倍率视角',
      viewMode_default: '默认（1.0x）',
      viewMode_group: '按分组',
      viewMode_account: '按账号',
      viewModeHint: '默认显示官方原价（倍率 1.0）。选择"按分组/按账号"叠加对应 rate_multiplier 后查看乘完倍率的最终单价。',
      selectGroup: '选择分组',
      selectAccount: '选择账号',
      searchModel: '搜索模型',
      searchPlaceholder: '模型名 / 供应商关键字',
      effectiveMultiplier: '当前倍率',
      col: {
        model: '模型',
        provider: '供应商',
        input: 'Input ($/MTok)',
      ...
      flagCachingTip: '该模型支持 prompt caching',
    ...
    accounts: {
      ...
      subscriptionTierPlaceholder: '选择档位（可选）',
      subscriptionTierHint: '标注后会出现在「订阅定价助手」的统计里，用于按档位聚合用量、辅助你给套餐定额。',
      subscriptionTierBadge: '定价统计',
      subscriptionTierOptional: '可选',
      subscriptionTierDescriptions: {
        unclassified: '暂不参与档位聚合，适合还没确认来源的账号',
        free: '免费或试用账号，作为容量基准单独统计',
        paid: '常规付费档位，用于估算主力套餐容量',
        high: '高容量档位，适合单独做高配套餐测算',
        team: '团队或企业档位，建议与个人档位分开统计',
      },
      tierUnclassified: '未分类',
      tierMissing: '未标档位',
      tierMissingHint: '该订阅账号还没标订阅档位，定价助手无法把它纳入统计。点编辑设置档位。',
      // Filter options
      allPlatforms: '全部平台',
      allTypes: '全部类型',
      allStatus: '全部状态',
      ...
      bulkResetStatusConfirm: '将清除选中的 {count} 个账号的错误状态，使其重新可调度。继续吗？',
      bulkRefreshTokenTitle: '批量刷新令牌',
      bulkRefreshTokenConfirm: '将为选中的 {count} 个账号刷新访问令牌；失败的账号会标记错误状态。继续吗？',
    ...
    ops: {
      ...
      // System Log
      systemLog: {
        resetRuntimeConfirm: '确认回滚为启动配置（env/yaml）并立即生效？',
        ...
        empty: '暂无系统日志',
        loadFailed: '系统日志加载失败',
        healthQueue: '队列 {depth}/{capacity}',
        healthWritten: '写入 {count}',
        healthDropped: '丢弃 {count}',
        healthFailed: '失败 {count}',
        lastWriteError: '最近写入错误：{error}',
        runtimeTitle: '运行时日志配置',
        runtimeBadge: '实时生效',
        fieldLevel: '级别',
        fieldStacktraceLevel: '堆栈阈值',
        fieldSamplingInitial: '采样初始',
        fieldSamplingThereafter: '采样后续',
        fieldRetentionDays: '保留天数',
        btnSaving: '保存中...',
        btnSaveApply: '保存并生效',
        btnResetDefaults: '回滚默认值',
        runtimeConfigSaved: '日志运行时配置已生效',
        runtimeConfigSaveFailed: '保存日志配置失败',
        runtimeConfigResetDone: '已回滚到启动日志配置',
        runtimeConfigResetFailed: '回滚日志配置失败',
        filterTimeRange: '时间范围',
        filterStartTime: '开始时间（可选）',
        filterEndTime: '结束时间（可选）',
        filterLevel: '级别',
        filterComponent: '组件',
        filterComponentPlaceholder: '如 http.access',
        filterPlatform: '平台',
        filterModel: '模型',
        filterKeyword: '关键词',
        filterKeywordPlaceholder: '消息/request_id',
        levelAll: '全部',
        btnSearch: '查询',
        btnReset: '重置',
        btnCleanupByFilter: '按当前筛选清理',
        btnRefreshHealth: '刷新健康指标',
    ...
    settings: {
      ...
      features: {
        ...
        invoice: {
          title: '发票管理',
          description: '允许用户对已完成的付费订单申请开票。开票金额按用户实付金额计算，不含赠送、折扣。默认关闭。',
          enabled: '启用发票功能',
          enabledHint: '关闭后用户端「我的发票」入口隐藏，开票相关接口返回未启用。',
        },
        affiliate: {
          title: '邀请返利',
      ...
      registration: {
        ...
        ipLimitTitle: '注册 IP 限流（反薅羊毛）',
        ipLimitHint: '在路由默认 5 次/分钟兜底之外，按更长时间窗口限制单 IP 注册。配合 Gmail 别名归一化能有效阻挡批量薅新人奖励。',
        ipLimitMaxCount: '窗口内最大注册次数',
        ipLimitMaxPlaceholder: '0 表示关闭',
        ipLimitMaxNote: '0 = 关闭业务级限流；推荐 3（每 IP 每小时最多 3 次）',
        ipLimitWindow: '窗口长度（分钟）',
        ...
        aiReviewTestButton: '测试 AI 审核',
        aiReviewTestTitle: 'AI 审核测试',
        aiReviewTestHint: '输入一段文本，立即看当前配置的模型 + prompt 会怎么判定。结果不入库，仅用于配置验证。',
        aiReviewTestPromptLabel: '测试内容',
        aiReviewTestPlaceholder: '粘贴一段被风控规则命中的用户请求，看 AI 是否能正确判定为 pass / flag / reject',
        aiReviewTestRun: '运行测试',
        passwordReset: '忘记密码',
      ...
      site: {
        ...
        contactMethodLabel: '显示名称',
        contactMethodValue: '账号/号码',
      ...
      payment: {
        ...
        quickAmounts: '快捷充值金额',
        quickAmountsPlaceholder: '例如 10,20,50,100,200',
        quickAmountsHint: '用英文逗号分隔，最多 {max} 个金额；留空时使用默认档位。',
        quickAmountsPreview: '前台预览',
        quickAmountsFormatError: '快捷充值金额格式不正确，请输入正数，最多 {max} 个，用英文逗号分隔。',
        orderTimeout: '订单超时时间',
  ...
  exclusiveSeats: {
    ...
    renew: '续费',
    renewTitle: '续费独享名额',
    renewHint: '为「{name}」延长有效期，绑定账号保持不变。',
    renewPaymentHint: '续费需要重新支付套餐金额；点击「前往支付」跳转到购买页继续。',
    priceIncreasedWarning: '套餐价格已上调 ¥{delta}，请确认本次续费金额后再支付。',
    priceDecreasedNote: '套餐价格已下调 ¥{delta}，本次续费按当前优惠价收取。',
    priceUnchangedNote: '套餐价格与上次一致，续费金额不变。',
    goToPay: '前往支付',
    usageGuideTitle: '需要 ApiKey 才能使用您的独享号',
    usageGuideHint: '请在「API 密钥」页创建一把绑定到对应分组的 ApiKey，发起请求时调度器会优先命中您的独享账号。',
    goToKeys: '前往 ApiKey',
    validityDays: '续费天数',
    status: {
      active: '生效中',
  ...
  invoice: {
    ...
    actions: {
      cancel: '取消申请',
      confirmCancel: '确定取消该开票申请吗？取消后关联订单将可重新申请。',
    },
    apply: {
      button: '申请开票',
      title: '申请开票',
      availableTitle: '当前可开票金额',
      availableHint: '按你在「支持开票分组」的真实消费核定；充值赠送、对接分组的消费不计入。',
      amountLabel: '开票金额（元）',
      amountPlaceholder: '留空 = 按可开票金额全额开票',
      amountHint: '可填写小于可开金额做部分开票，剩余额度下次仍可开。',
      submit: '提交申请',
      success: '开票申请已提交',
      companyNamePlaceholder: '请输入企业名称',
      personalNamePlaceholder: '请输入个人名称（默认「个人」）',
      taxIdPlaceholder: '请输入纳税人识别号',
      emailPlaceholder: '用于接收电子发票的邮箱',
      remarkPlaceholder: '选填，如有特殊开票要求请说明',
    },
    summary: {
      title: '可开票额度',
      balance: '余额消费可开',
    ...
    detail: {
      title: '发票详情',
      relatedOrders: '关联订单',
      emailSent: '发票已发送至接收邮箱',
      emailPending: '发票已开具，邮件发送中',
    },
    admin: {
      searchPlaceholder: '搜索抬头/邮箱/税号',
      applicant: '申请人',
      issue: '开票',
      reject: '驳回',
      issueTitle: '开具发票',
      uploadPdf: '上传发票 PDF',
      uploadHint: 'PDF 仅用于发送给客户和识别号码，系统不会留存文件',
      chooseFile: '选择 PDF',
      changeFile: '重新选择',
      parsing: '识别中...',
      invoiceNumberPlaceholder: '识别失败时请手动填写',
      issueNote: '确认后将发票 PDF 发送至客户邮箱，并留存发票号码等信息（不留存 PDF 文件）',
      confirmIssue: '确认开票并发送',
      issuing: '开票中...',
      issueSuccess: '开票成功，发票已发送',
      rejectTitle: '驳回开票申请',
      rejectNote: '驳回后关联订单将释放，客户可重新申请。',
      rejectReasonPlaceholder: '请说明驳回原因，将告知客户',
    },
    errors: {
      INVOICE_NOT_FOUND: '开票申请不存在',
      INVOICE_NOT_OWNED: '无权操作该开票申请',
      INVOICE_NOT_PENDING: '该申请已处理，无法操作',
      INVOICE_NOTHING_TO_INVOICE: '当前没有可开票金额',
      INVOICE_AMOUNT_EXCEEDS_AVAILABLE: '开票金额超过可开额度，请刷新后重试',
      INVOICE_TAX_ID_REQUIRED: '企业抬头必须填写税号',
      INVOICE_INVALID_TITLE: '发票抬头不合法',
      INVOICE_INVALID_EMAIL: '接收邮箱格式不正确',
      INVOICE_NUMBER_REQUIRED: '请填写发票号码',
      INVOICE_PDF_REQUIRED: '请上传发票 PDF',
      INVOICE_PDF_TOO_LARGE: '发票 PDF 超过 10MB 限制',
      INVOICE_PDF_INVALID: '仅支持 PDF 文件',
      INVOICE_PDF_EMPTY: '发票 PDF 内容为空',
      INVOICE_DISABLED: '发票功能未开启',
    },
  },
  ...
  payment: {
    ...
    renewalBanner: {
      title: '续费独享名额',
      hint: '完成支付后自动延长有效期，原绑定账号不变。',
      hintUp: '套餐已涨价，请确认金额后再支付。',
      hintDown: '套餐已降价，按当前优惠价收取。',
      lastPaid: '上次支付',
      thisTime: '本次支付',
      priceUp: '上调 ¥{delta}',
      priceDown: '下调 ¥{delta}',
    ...
    orders: {
      ...
      requestRefund: '申请退款',
      viewRefund: '查看退款详情',
      refundDetailsTitle: '退款详情',
      refundAmountLabel: '退款金额',
      refundPartialNote: '原订单 ¥{paid}，本次部分退款',
      refundedAt: '退款时间',
      refundRequestedAt: '申请时间',
      refundOriginalRoute: '原路退回',
      refundProcessHint: '退款发起后通常 1-5 个工作日内退回您的支付账户。',
      refundReasonCodes: {
        exclusive_pool_sold_out: '独享池库存已售罄，已自动全额退款',
      },
    },
    result: {
      success: '支付成功',
    ...
    subscriptionGuide: {
      title: '订阅通常更划算',
      subtitle: '当前有 {count} 个订阅套餐可选。高频使用建议直接订阅，比反复充值更省心。',
      subtitleWithPrice: '当前有 {count} 个订阅套餐可选，最低 ¥{price} 起。高频使用建议直接订阅。',
      action: '去订阅',
    },
    tabTopUp: '充值',
    tabSubscribe: '订阅',
    noPlans: '暂无可用订阅套餐',
    noPlansHint: '管理员还没上架任何套餐。如需开通，请联系客服或稍后再来。',
    ...
    invoiceNotice: '开票说明：仅可对消费在「支持开票分组」的部分申请开票，充值赠送、兑换码不计入。充了未消费的余额暂不能开票，消费后即可开。',
    invoiceEligible: '本套餐消费支持开票',
    invoiceIneligible: '本套餐消费暂不支持开票，请知悉后再购买',
    refundReason: '退款原因',
    ...
    planCard: {
      ...
      models: '模型',
      tierBadge: '独立档位',
      tierBadgeHint: '该套餐自带限额配置，与同分组其他套餐不同',
      popularBadge: '主推',
      subscribed: '已订阅 · 剩 {days} 天',
    },
    days: '天',
    months: '个月',
    years: '年',
    ...
    admin: {
      ...
      planKind: '套餐类型',
      planKindShared: '共享池',
      planKindExclusive: '独享池',
      planKindSharedHint: '共享池套餐：用户购买后与其他用户共享 group 内账号资源。',
      planKindExclusiveHint: '独享池套餐：用户购买后从该 group 池子里独占分配一个账号，整个订阅期独享。',
      limitOverrideTitle: '套餐限额（覆盖分组默认）',
      limitOverrideHint: '所有字段留空表示沿用所在分组的默认配置；填值则覆盖。常用于同分组下做 Lite/Pro/Max 差异化档位。',
      planDailyLimitUSD: '日限额 (USD)',
      planWeeklyLimitUSD: '周限额 (USD)',
      planMonthlyLimitUSD: '月限额 (USD)',
      planRateMultiplier: '倍率',
      limitInheritGroup: '留空 = 沿用分组',
      limitHint: '0 或留空 = 无限制',
      warnWeeklyRedundant: '该值不生效：被日限额 × 7 覆盖（实际上限 ${max}）',
      warnMonthlyRedundant_daily: '该值不生效：被日限额 × 30 覆盖（实际上限 ${max}）',
      warnMonthlyRedundant_weekly: '该值不生效：被周限额 × 30/7 覆盖（实际上限 ${max}）',
      warnMonthlyRedundant_both: '该值不生效：被日 / 周限额共同覆盖（实际上限 ${max}）',
      kindBadgeExclusive: '独享',
      stockAvailable: '剩余 {n} 个名额',
      stockSoldOut: '已售罄',
      cardType: {
        all: '全部',
        daily: '日卡',
      ...
      exclusivePools: {
        title: '独享池管理',
        subtitle: '查看每个独享池的库存与用户名额，支持手动赠送 / 释放 / 换号 / 延期。',
        poolGroup: '选择独享池（分组）',
        grantButton: '赠送名额',
        grantTitle: '管理员赠送独享名额',
        targetUserID: '用户 ID',
        targetPlanID: '套餐 ID',
        validityDays: '有效期（天）',
        notes: '备注',
        noPlansForPool: '当前独享池下没有可赠送的套餐，请先在套餐管理里创建独享套餐并绑定到该池',
        planMustBelongToPool: '所选套餐不属于当前独享池，请重新选择本池下的套餐',
        userSearchPlaceholder: '输入用户 ID / 邮箱 / 用户名搜索',
        userSearchSelected: '已选',
        userSearchNoMatch: '没有匹配的用户，换个关键字试试',
        statTotal: '总账号',
        statFree: '空闲',
        statAvailableNow: '可立即分配',
        statFreeRaw: '空闲（含限流）{n}',
        statSchedulable: '可立即分配 {n}',
        statSchedulableHint: '剔除限流/过载/临时不可用账号后真实可售卖的数量',
        statUsed: '在用',
        statExpiring: '7 天内到期',
        statExpiringClickFilter: '点击筛选',
        statExpiringClickClear: '点击取消筛选',
        expiringFilterActive: '已筛选 7 天内到期且仍在使用中的名额',
        expiringFilterClear: '取消筛选',
        emptyExpiring: '没有 7 天内到期的活跃名额',
        colUser: '用户',
        colAccount: '账号',
        colStatus: '状态',
        colExpiresAt: '到期时间',
        ...
        actSwap: '换号',
        actRelease: '释放',
        releaseTitle: '强制释放独享名额',
        releaseHint: '即将释放 #{id}，绑定账号会立刻回池。',
        reason: '原因',
        extendTitle: '延期 / 提前结束',
        extendDays: '天数（正数延期，负数提前）',
        extendHint: '延期不影响绑定账号；提前结束不允许导致到期时间在过去。',
        extendPreviewCurrent: '当前到期',
        extendPreviewDelta: '变更',
        extendPreviewNew: '新到期时间',
        extendPreviewInvalidPast: '调整后到期时间已是过去，不允许提交',
        daysSuffix: '天',
        swapConfirm: '确定要从池中分配一个新空闲账号给该名额吗？原账号会立刻回池。',
        swapTitle: '换号确认',
        swapDialogHint: '本次操作会从池中分配一个新的空闲账号给该名额，并立刻把当前绑定账号释放回池。',
        swapDialogOldAccount: '当前账号',
        swapDialogWarn: '换号会让用户的会话/缓存可能立即失效，请确认确实需要。',
      },
      createPlan: '创建套餐',
      editPlan: '编辑套餐',
      deletePlan: '删除套餐',
      ...
      planLabel: '档位名',
      planLabelPlaceholder: '留空自动推导（如 Lite / Pro）',
      planLabelHint: '订阅页档位对比表的列头，留空则从套餐名自动提取。',
      planBadgeText: '套餐标签',
      ...
      adminOnly: '仅后台可见',
      quotaPriceRate: '售价/额度',
      billingRate: '计费倍率',
      effectiveCostRate: '最终倍率',
      costMultiplierUnavailable: '填写价格和同周期日/周/月 USD 限额后自动换算。',
      periodQuotaUsed: '使用同周期额度：{quota}',
      costMultiplierTitle: '额度 {quota}；售价/额度 {quotaRate}；计费 {billingRate}；最终 {effectiveRate}',
      targetCostMultiplier: '目标成本倍率',
      targetCostMultiplierPlaceholder: '例如 1.25',
      periodQuotaInput: '同周期额度',
      periodQuotaInputPlaceholder: '自动额度 / 手填额度',
      costCalculatorHint: '公式：价格 / 同周期额度 × 计费倍率 = 目标成本倍率。',
      costCalculatorTarget: '将写入 {field}：{value}。',
      costCalculatorUnavailable: '先设置有效周期后才能使用换算。',
      isPopular: '设为主推',
      isPopularHint: '勾选后用户端订阅卡显示"主推"徽章 + 琥珀色描边',
      popularBadgeShort: '主推',
      bulkSelected: '已选 {n} 个',
      bulkSelectAll: '选择当前可见的全部',
      bulkActionEnable: '批量上架',
      bulkActionDisable: '批量下架',
      bulkActionDone: '操作完成，{n} 个成功',
      bulkActionPartialFail: '{n} 个操作失败，请重试',
      ...
      daySuffix: '天',
      financeTitle: '财务收入统计',
      financeDesc: '按财务入账口径统计真实收入，支持按月或按日查看',
      financePeriodMonth: '按月',
      financePeriodDay: '按日',
      financeMonth: '统计月份',
      financeDate: '统计日期',
      financeNetIncome: '真实收入',
      financeGrossIncome: '实收总额',
      financeRefunds: '退款扣减',
      financeAvgOrder: '平均实收',
      financeLedgerNote: '口径：仅统计已完成订单和部分退款后的净额，全额退款、待支付、退款中订单不计入真实收入。',
      colDate: '日期',
      netIncome: '真实收入',
      grossIncome: '实收总额',
      refundDeduction: '退款扣减',
