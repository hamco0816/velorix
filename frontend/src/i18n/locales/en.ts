export default {
  ...
  admin: {
    ...
    pricingAdvisor: {
      ...
      helpBody: 'Stats are the rolling 7-day peak cost per account over the past N days. Treat them as "single-account weekly peak load", multiply by safety and divide by your target per-user monthly limit to derive how many users one account can serve and the suggested price.',
      ...
      calculator: {
        ...
        targetUserLimit: 'Target per-user monthly limit (USD)',
        targetUserLimitTip: 'How much you want each user to spend monthly (token-USD). The system derives how many users one account can serve and the suggested price.',
        margin: 'Target margin',
        marginTip: 'Target gross margin. e.g. cost ¥1400, 30% margin → revenue target ¥1820, divided by derived user count = per-user monthly price.',
        safety: 'Safety factor',
        safetyTip: '< 1 limits to a fraction of P50. Lower = safer, less cap-hit risk, but users may feel "not enough". Recommend 0.7-0.85.',
        selectFirst: 'Pick a tier first, then fill in the parameters.',
        resultTitle: 'Suggested plan',
        suggestedPrice: 'Suggested price',
        derivedUsers: 'Estimated users per account',
        derivedUsersTip: 'Based on tier P50 7d peak × 4 (monthly proxy) × safety ÷ your per-user limit.',
        ...
        warnOverP95: 'Estimated total usage exceeds historical P95; multiple users may hit the cap simultaneously during peaks.',
        warnSamplesLow: 'Sample size is small (< 3 accounts). Results are rough; collect more data before final decisions.',
        warnTooFewUsers: 'Your per-user limit is too high — one account would serve < 1 user. Lower the limit or pick a higher tier.',
      },
