export default {
  ...
  admin: {
    ...
    pricingAdvisor: {
      ...
      helpBody: 'The calculator prefers reverse-derived caps: cap = current-window cost ÷ upstream utilization%. e.g. 7d window at 50%, $10 spent → cap = $20. Then split across N users: per-user weekly = cap × safety / N, daily = weekly / 7, monthly = weekly × 4. The 5h / 7d table columns are sliding-window peaks over the last N days for reference; when a tier has no utilization sample, cap falls back to 7d_P95.',
      ...
      tip: {
        samples: 'Number of accounts in this tier with usage data. Stats are meaningful at ≥ 3 samples; smaller samples get a ⚠ on P95.',
        ...
        dailyAvg: 'Average daily cost per account over the past N days. Approximate steady-state consumption.',
      ...
      samples: 'samples',
      calculator: {
        title: 'ROI / quota calculator',
        tier: 'Tier',
        cost: 'Account cost (¥/mo)',
        usersPerAccount: 'Users per account (N)',
        usersPerAccountTip: 'How many subscribers will share one account. The upstream cap × safety is split across N users to derive each user\'s 5h / daily / weekly / monthly quota; price/user = cost × (1+markup) ÷ (N × occupancy).',
        markup: 'Markup',
        markupTip: 'Markup = profit / cost. e.g. cost ¥1400, 30% markup → revenue target ¥1820. Note this is markup, not margin (margin = profit/revenue = 23%).',
        occupancy: 'Occupancy',
        occupancyTip: 'Expected fraction of N seats actually sold. Below 100% the price is bumped up so fewer seats still cover cost. e.g. N=10, occupancy 80% → cost split across 8 paying users.',
        safety: 'Safety factor',
        safetyTip: '< 1 limits to a fraction of cap. Lower = safer, less cap-hit risk, but users may feel "not enough". Recommend 0.7-0.85.',
        rateMultiplier: 'Plan rate multiplier',
        rateMultiplierTip: 'rate_multiplier of the plan\'s group. User\'s billed cost = upstream cost × multiplier, so when > 1 the user-side limit must scale up accordingly. Default 1 (leaving it unchanged has no effect).',
        capOverrideTitle: 'Advanced: manually override cap',
        capOverrideHint: 'The system reverse-derives cap from utilization, falling back to 7d_P95 when no sample exists. If you know the real upstream cap (e.g. official docs say Pro 20x weekly cap is $4000), enter it here. Empty = auto.',
        capOverride5h: '5h cap (USD)',
        capOverride7d: '7d cap (USD)',
        ...
        profitTip: 'Monthly profit at current occupancy = price/user × N × occupancy − account cost. Full-occupancy (100%) profit: ¥{full}. Negative means current price + occupancy can\'t cover account cost.',
        perMonth: 'mo',
        limitsTitle: 'Per-user subscription limits (USD)',
        limitsTip: '5h limit = cap_5h × safety × rate_multiplier / N; weekly = cap_7d × safety × rate_multiplier / N; daily = weekly/7; monthly = weekly×4. cap priority: manual override > utilization-reversed > 7d_P95 fallback.',
        limitsSourceLine: 'cap_5h={cap5h}$ ({cap5hSrc}) · cap_7d={cap7d}$ ({cap7dSrc}), × safety {safety} × multiplier {mul} ÷ N={n}',
        capSource_override: 'manual',
        capSource_reversed: 'utilization-reversed',
        capSource_fallback: '7d_P95 fallback',
        limitsCapsIndependentNote: 'Note: the 4 limits are independent caps; whichever the user hits first is binding. 5h is reference-only — the plan model doesn\'t store 5h, enforce it on the upstream subscription side.',
        applyToPlan: 'Create plan with this suggestion',
        warnNoCapSample: 'No utilization sample for this tier — falling back to 7d_P95 estimate. If the account hasn\'t been pushed to limit, this fallback will be much lower than the real cap. Recommend entering it manually under "Advanced".',
        ...
        warnUnprofitable: 'At current price + occupancy, account cost cannot be covered. Raise markup, raise occupancy expectation, or raise N.',
      },
