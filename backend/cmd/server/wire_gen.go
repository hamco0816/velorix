...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	supportHub := service.NewSupportHub()
	supportService := service.NewSupportService(db, supportHub)
	...
	adminSupportHandler := admin.NewSupportHandler(supportService)
	...
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, apiKeyRepository)
	...
	seatHandler := admin.NewSeatHandler(paymentService, paymentConfigService)
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminService)
	pricingAdvisorService := service.NewPricingAdvisorService(db)
	pricingAdvisorHandler := admin.NewPricingAdvisorHandler(pricingAdvisorService)
	invoiceService := service.NewInvoiceService(client, emailService, settingService)
	invoiceHandler := admin.NewInvoiceHandler(invoiceService)
	desktopReleaseRepository := repository.NewDesktopReleaseRepository(client)
	desktopReleaseService := service.NewDesktopReleaseService(desktopReleaseRepository, configConfig)
	desktopReleaseHandler := admin.NewDesktopReleaseHandler(desktopReleaseService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, adminSupportHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, paymentHandler, seatHandler, affiliateHandler, pricingAdvisorHandler, invoiceHandler, desktopReleaseHandler)
	...
	handlerSeatHandler := handler.NewSeatHandler(paymentService, paymentConfigService)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	handlerInvoiceHandler := handler.NewInvoiceHandler(invoiceService, settingService)
	desktopHandler := handler.NewDesktopHandler(desktopReleaseService)
	idempotencyRepository := repository.NewIdempotencyRepository(client, db)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, supportHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, handlerSeatHandler, availableChannelHandler, handlerInvoiceHandler, desktopHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig)
	systemOverloadMonitor := service.ProvideSystemOverloadMonitor(settingService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, systemOverloadMonitor, redisClient)
	...
	exclusiveSeatExpiryService := service.ProvideExclusiveSeatExpiryService(paymentService)
	seatReleaseRetryService := service.ProvideSeatReleaseRetryService(paymentService)
	schedulerOutboxNotifier := repository.NewSchedulerOutboxNotifier(db)
	exclusiveSeatGatewayWired := service.ProvideExclusiveSeatGatewayWiring(gatewayService, openAIGatewayService, geminiMessagesCompatService, paymentService, billingCacheService, subscriptionService, schedulerOutboxNotifier)
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository)
	scheduledTestRunnerService := service.ProvideScheduledTestRunnerService(scheduledTestPlanRepository, scheduledTestService, accountTestService, rateLimitService, configConfig)
	paymentOrderExpiryService := service.ProvidePaymentOrderExpiryService(paymentService)
	channelMonitorRunner := service.ProvideChannelMonitorRunner(channelMonitorService, settingService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, schedulerSnapshotService, tokenRefreshService, accountExpiryService, exclusiveSeatExpiryService, seatReleaseRetryService, systemOverloadMonitor, exclusiveSeatGatewayWired, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner)
