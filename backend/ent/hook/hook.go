...
// The DesktopReleaseFunc type is an adapter to allow the use of ordinary
// function as DesktopRelease mutator.
type DesktopReleaseFunc func(context.Context, *ent.DesktopReleaseMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DesktopReleaseFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DesktopReleaseMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DesktopReleaseMutation", m)
}
