...
// The ExclusiveSubscriptionFunc type is an adapter to allow the use of ordinary function as a Querier.
type ExclusiveSubscriptionFunc func(context.Context, *ent.ExclusiveSubscriptionQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ExclusiveSubscriptionFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ExclusiveSubscriptionQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ExclusiveSubscriptionQuery", q)
}

// The TraverseExclusiveSubscription type is an adapter to allow the use of ordinary function as Traverser.
type TraverseExclusiveSubscription func(context.Context, *ent.ExclusiveSubscriptionQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseExclusiveSubscription) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseExclusiveSubscription) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ExclusiveSubscriptionQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ExclusiveSubscriptionQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ExclusiveSubscriptionQuery:
		return &query[*ent.ExclusiveSubscriptionQuery, predicate.ExclusiveSubscription, exclusivesubscription.OrderOption]{typ: ent.TypeExclusiveSubscription, tq: q}, nil
	case *ent.GroupQuery:
