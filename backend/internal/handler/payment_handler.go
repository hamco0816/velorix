...
func (h *PaymentHandler) GetPlansPublic(c *gin.Context) {
	...
	for _, p := range plans {
		result = append(result, publicPlan{
			...
			IsPopular: p.IsPopular, BadgeText: p.BadgeText, BadgeColor: p.BadgeColor, PlanLabel: p.PlanLabel, TierStyle: p.TierStyle,
		})
	}
...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	for _, p := range plans {
		row := planWithPlatform{
			...
			IsPopular: p.IsPopular, BadgeText: p.BadgeText, BadgeColor: p.BadgeColor, PlanLabel: p.PlanLabel, TierStyle: p.TierStyle, Kind: p.Kind,
		}
		if p.Kind == "exclusive" {
