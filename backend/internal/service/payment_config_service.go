...
type CreatePlanRequest struct {
	...
	// PlanLabel 档位名（对比表列头用，如 Lite/Pro），空 = 前端从套餐名自动推导
	PlanLabel string `json:"plan_label"`
	// TierStyle 档位样式预设 key（basic/standard/advanced/flagship/luxury/supreme），空 = basic
	TierStyle string `json:"tier_style"`
...
func (s *PaymentConfigService) GetPaymentConfig(ctx context.Context) (*PaymentConfig, error) {
	keys := []string{
		...
		SettingEnabledPaymentTypes, SettingBalancePayDisabled, SettingBalanceRechargeMult, SettingRechargeFeeRate, SettingQuickAmounts, SettingLoadBalanceStrategy,
...
func (s *PaymentConfigService) parsePaymentConfig(vals map[string]string) *PaymentConfig {
	cfg := &PaymentConfig{
		...
		QuickAmounts:              parsePaymentQuickAmounts(vals[SettingQuickAmounts]),
...
func (s *PaymentConfigService) UpdatePaymentConfig(ctx context.Context, req UpdatePaymentConfigRequest) error {
	...
	if req.QuickAmounts != nil {
		var err error
		quickAmountsValue, err = formatPaymentQuickAmounts(req.QuickAmounts)
		if err != nil {
			return err
		}
	}
	...
	if req.QuickAmounts != nil {
		m[SettingQuickAmounts] = quickAmountsValue
	}
...
func defaultPaymentQuickAmounts() []float64 {
	return []float64{10, 20, 50, 100, 200, 500, 1000, 2000, 5000}
}

func DefaultPaymentQuickAmounts() []float64 {
	return defaultPaymentQuickAmounts()
}

func parsePaymentQuickAmounts(raw string) []float64 {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return defaultPaymentQuickAmounts()
	}
	values, err := normalizePaymentQuickAmounts(strings.Split(raw, ","))
...
func formatPaymentQuickAmounts(values []float64) (string, error) {
	if len(values) == 0 {
		return "", nil
	}
	raw := make([]string, 0, len(values))
	for _, value := range values {
		raw = append(raw, strconv.FormatFloat(value, 'f', -1, 64))
	}
	values, err := normalizePaymentQuickAmounts(raw)
	if err != nil {
		return "", err
	}
...
func normalizePaymentQuickAmounts(tokens []string) ([]float64, error) {
	if len(tokens) > maxPaymentQuickAmounts {
		return nil, infraerrors.BadRequest("INVALID_PAYMENT_QUICK_AMOUNTS", fmt.Sprintf("quick amounts can contain at most %d values", maxPaymentQuickAmounts))
	}
	...
	for _, token := range tokens {
		...
		if err != nil || math.IsNaN(value) || math.IsInf(value, 0) || value <= 0 {
			return nil, infraerrors.BadRequest("INVALID_PAYMENT_QUICK_AMOUNTS", "quick amounts must be positive numbers")
		}
		cents := math.Round(value * 100)
		if math.Abs(value*100-cents) > 1e-9 {
			return nil, infraerrors.BadRequest("INVALID_PAYMENT_QUICK_AMOUNTS", "quick amounts allow at most 2 decimal places")
		}
		key := int64(cents)
		if _, ok := seen[key]; ok {
			continue
		...
		seen[key] = struct{}{}
		values = append(values, cents/100)
	}
	...
	for _, inst := range instances {
		...
		case payment.TypeXunhupay:
			for _, supportedType := range splitTypes(inst.SupportedTypes) {
				switch NormalizeVisibleMethod(supportedType) {
				case payment.TypeAlipay:
					available[VisibleMethodSourceXunhupayAlipay] = true
				case payment.TypeWxpay:
