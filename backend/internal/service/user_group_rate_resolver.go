...
func userGroupRateCacheEntryFromRepoValue(userRate *float64) userGroupRateCacheEntry {
	if userRate == nil {
		return userGroupRateCacheEntry{}
	}
	rate := *userRate
	return userGroupRateCacheEntry{UserRate: &rate}
}
...
func (r *userGroupRateResolver) Resolve(ctx context.Context, userID, groupID int64, groupDefaultMultiplier float64) float64 {
	...
	value, err, shared := r.sf.Do(key, func() (any, error) {
		...
		entry := userGroupRateCacheEntryFromRepoValue(userRate)
		if r.cache != nil {
